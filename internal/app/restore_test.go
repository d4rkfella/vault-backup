...
func TestRestore_Fail_ResolveBackupKey_Error(t *testing.T) {
	ctx := context.Background()
	vaultMock := newMockVaultClient(t)
	s3Mock := new(mockS3Client)
	notifyMock := new(mockNotifyClient)
	expectedError := errors.New("s3 resolve key failed")

	s3Mock.On("ResolveBackupKey", ctx).Return("", expectedError).Once()
	...
	notifyMock.On("Notify", ctx,
		...
		int64(0),
		mock.MatchedBy(func(err error) bool { return errors.Is(err, expectedError) }),
